...
function renderInvestments() {
  ...
  // Mobile cards → inv-list-mobile | Desktop rows → inv-table
  var mobileCards = [];
  var desktopRows = [];
  ...
  state.investments.forEach(function(i) {
      ...
      '<div style="padding:14px 16px;border-bottom:1px solid var(--border)">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">' +
          '<div style="display:flex;align-items:center;gap:10px">' +
            ...
            '<div>' +
              ...
              '<div style="font-size:11px;color:var(--text-muted)">' + i.type.toUpperCase() + ' · ' + iCurr + ' · Qty ' + i.qty.toLocaleString() + '</div>' +
            '</div>' +
          '</div>' +
        ...
        '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">' +
          ...
          '<div style="background:' + (i.fetchFailed ? 'rgba(248,113,113,0.08)' : 'var(--bg-elevated)') + ';border-radius:8px;padding:8px;border:' + (i.fetchFailed ? '1px solid rgba(248,113,113,0.3)' : 'none') + '">' +
            '<div style="font-size:10px;color:var(--text-muted);margin-bottom:2px">Live Price' + (liveTime ? ' · ' + liveTime : '') + '</div>' +
            (i.fetchFailed
              ? '<div style="font-size:11px;color:var(--red);font-weight:600">⚠️ Manual</div><button onclick="openManualPriceEdit(\'' + i.id + '\')" style="margin-top:4px;font-size:10px;padding:2px 6px;border-radius:4px;background:var(--red);color:white;border:none;cursor:pointer">Update</button>'
    ...
    desktopRows.push(
      '<tr>' +
      '<td class="td-primary"><div style="display:flex;align-items:center;gap:7px"><span class="type-dot" style="background:' + col + ';flex-shrink:0"></span><div><div style="font-weight:700">' + i.name + '</div><div style="font-size:10px;color:var(--text-muted)">' + iCurr + '</div></div></div></td>' +
      ...
      '<td><div style="display:flex;align-items:center;gap:5px"><span class="td-mono">' + fmtInv(i.currentPrice, iCurr, priceDecimals) + '</span>' + (i.lastUpdated && !i.fetchFailed ? '<span class="live-dot"></span>' : '') + '</div>' +
        (i.fetchFailed ? '<div style="font-size:10px;color:var(--red);margin-top:1px">⚠️ <span style="cursor:pointer;text-decoration:underline" onclick="openManualPriceEdit(\'' + i.id + '\')">Update</span></div>' : (i.lastUpdated ? '<div style="font-size:10px;color:var(--text-muted);margin-top:1px">Live · ' + liveTime + '</div>' : '<div style="font-size:10px;color:var(--text-muted);margin-top:1px">Manual</div>')) + '</td>' +
      '<td class="td-mono td-primary">' + fmtInv(val, iCurr) + '</td>' +
      '<td><span class="td-mono pnl-amount" style="font-size:12px;font-weight:700;color:' + pnlColor + '">' + pnlSign + fmtInv(pnl, iCurr) + '</span><div style="font-size:11px;font-weight:700;color:' + pnlColor + ';margin-top:1px">' + pnlSign + pnlPct.toFixed(2) + '%</div></td>' +
      '<td><div style="display:flex;gap:4px"><button class="btn btn-ghost btn-sm btn-icon" onclick="editInvestment(\'' + i.id + '\')"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" style="width:13px;height:13px"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/></svg></button><button class="btn btn-danger btn-sm btn-icon" onclick="deleteInvestment(\'' + i.id + '\')"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" style="width:13px;height:13px"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"/></svg></button></div></td>' +
      '</tr>'
    );
  });
...
window.renderWallet = function() {
  ...
  el.innerHTML =
    // Net worth summary — white card with green top accent
    '<div class="card" style="margin-bottom:14px;background:var(--bg-card);border-top:4px solid #1D9E75;border-radius:0 0 14px 14px">' +
      '<div style="font-size:10px;font-weight:700;color:#0F6E56;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px">Total Net Worth</div>' +
      ...
      '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px">' +
        '<div style="font-size:12px;color:var(--text-muted)">Cash <span style="color:var(--text-primary);font-weight:600">' + sym + totalCash.toLocaleString('en-US',{maximumFractionDigits:0}) + '</span></div>' +
        '<div style="font-size:12px;color:var(--text-muted)">Banks <span style="color:var(--text-primary);font-weight:600">' + sym + totalBank.toLocaleString('en-US',{maximumFractionDigits:0}) + '</span></div>' +
        '<div style="font-size:12px;color:var(--text-muted)">Investments <span style="color:var(--text-primary);font-weight:600">' + sym + totalInvest.toLocaleString('en-US',{maximumFractionDigits:0}) + '</span></div>' +
        '<div style="font-size:12px;color:var(--text-muted)">Assets <span style="color:var(--text-primary);font-weight:600">' + sym + totalAssets.toLocaleString('en-US',{maximumFractionDigits:0}) + '</span></div>' +
        '<div style="font-size:12px;color:var(--text-muted)">Liabilities <span style="color:var(--red);font-weight:600">-' + sym + totalLiab.toLocaleString('en-US',{maximumFractionDigits:0}) + '</span></div>' +
      '</div>' +
    '</div>' +
